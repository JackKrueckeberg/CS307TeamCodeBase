...
export const Login = () => {
  ...
  const validateToken = async (token) => {
    try {
      const response = await fetch(
        "http://localhost:5050/loginRoute/validate-token",
        {
          method: "GET",
          headers: {
  ...
  const handleForgotPasswordSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5050/emailForgotPassword/send-reset-email",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <PageAnimation>
      <div className={styles.userAuthentication}>
        ...
        <form onSubmit={submission} className={styles.form}>
          ...
          {incorrectAttempts > 0 && (
            <p className={styles.incorrect}>Incorrect email or password</p>
          )}
